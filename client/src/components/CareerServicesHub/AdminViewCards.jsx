...
function AdminViewCards({
  ...
  const handleRead = (index)=>{
    ...
    const fetchMessage = async ()=>{
      ...
      const result = await response.json();
      setMessageList(messageList.slice(0,index).concat(messageList.slice(index+1)))
    } 
    fetchMessage();
  }
  const handleClear = () => {
  ...
  function handleFilterToggle() {
    ...
    return (
        ...
        <div className='admin-view-notification'>
          <div id='notification-bell' onClick={()=>setShowNotify((prev)=>!prev)}>
            {messageList.length<1?null:<div className='notificationsCount'><span style={{marginBottom:'3px'}}>{messageList.length}</span></div>}
          ...
          {!showNotify || messageList.length < 1?null:(
            <div className='notificationPanel'>
              {messageList.map((element,index)=>(
                <div key={index}> 
                  <p>{element.message}</p>
                  <p id='viewed-button' ref={buttonRef} onClick={()=>handleRead(index)}>mark as viewed</p>
                </div>
              ))}
            </div>
