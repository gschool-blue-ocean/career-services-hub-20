...
export default function StudentChangeCard({
  currentStudent,
  url,
  setUpdate,
  socketRef,
  ...
  const handleSubmitResources = () => {
    const fetchData = async () => {
      const response = await fetch(
        `${url}/students/${currentStudent.student_id}`,
        {
          method: "PATCH",
          body: JSON.stringify({
      ...
      <div className="student-card-item">
        ...
        <div>
          ...
          <div>
            ...
            <select
              defaultValue={hunterAccess}
              onChange={(e) => setHunterAccess(e.target.value)}
            >
              {milestoneProgressOptions.map((element, index) => (
                <option key={index} value={element}>
                  {element}
